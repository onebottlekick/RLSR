...
STEPS = FLAG.steps
MODEL_PATH = os.path.join(CKPT_DIR, f"RLSR-x{SCALE}.pt")
...
def main():
    ...
    MODEL = Agent(N_ACTIONS).to(DEVICE)
    OPTIMIZER = torch.optim.Adam(MODEL.parameters(), LEARNING_RATE)
    RLSR = A3C(MODEL, T_MAX, GAMMA, BETA)
    RLSR.setup(SCALE, OPTIMIZER, BATCH_SIZE, PSNR,  DEVICE, MODEL_PATH, CKPT_DIR)
    # RLSR.load_checkpoint(CKPT_PATH)
    RLSR.train(train_set, valid_set, BATCH_SIZE, STEPS, SAVE_EVERY, SAVE_LOG)
